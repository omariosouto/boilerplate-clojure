...
(defflow check-home-status-flow-version
  (flow "check home status"
        ...
        (match? {:version "0.0.1"} (json/read-str (:body (components/test-request :get "/api/version")) :key-fn keyword))))

(deftest check-home-status
  (let [response (components/test-request :get "/api/version")]
    (is (= 200 (:status response)))
    (is (= {:version "0.0.1"} (json/read-str (:body response) :key-fn keyword)))))
